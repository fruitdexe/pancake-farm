...
contract('MasterChef', ([alice, bob, carol, dev, , treasury, minter]) => {
    beforeEach(async () => {
        this.milk = await MilkToken.new(treasury, { from: minter });
        ...
        this.chef = await MasterChef.new(this.milk.address, this.foam.address, dev, '1000', '100', '1', { from: minter });
    ...
    it('real case', async () => {
      ...
      await time.advanceBlockTo('687');
      await this.lp1.approve(this.chef.address, '1000', { from: alice });
      assert.equal((await this.milk.balanceOf(alice)).toString(), '0');
      await this.chef.deposit(1, '20', { from: alice });
      await this.chef.withdraw(1, '20', { from: alice });
      assert.equal((await this.milk.balanceOf(alice)).toString(), '263');
      ...
      await this.chef.enterStaking('0', { from: alice });
      await this.chef.enterStaking('0', { from: alice });
      assert.equal((await this.milk.balanceOf(alice)).toString(), '993');
    ...
    it('deposit/withdraw', async () => {
      ...
      assert.equal((await this.milk.balanceOf(alice)).toString(), '999');
      assert.equal((await this.milk.balanceOf(dev)).toString(), '100');
      ...
      assert.equal((await this.milk.balanceOf(bob)).toString(), '125');
    ...
    it('staking/unstaking', async () => {
      ...
      await this.milk.approve(this.chef.address, '250', { from: alice });
      await this.chef.enterStaking('240', { from: alice }); //3
      assert.equal((await this.foam.balanceOf(alice)).toString(), '240');
      assert.equal((await this.milk.balanceOf(alice)).toString(), '10');
      await this.chef.enterStaking('10', { from: alice }); //4
      assert.equal((await this.foam.balanceOf(alice)).toString(), '250');
      assert.equal((await this.milk.balanceOf(alice)).toString(), '249');
      await this.chef.leaveStaking(250);
      assert.equal((await this.foam.balanceOf(alice)).toString(), '0');
      assert.equal((await this.milk.balanceOf(alice)).toString(), '749');
    ...
    it('updaate multiplier', async () => {
      ...
      await this.milk.approve(this.chef.address, '100', { from: alice });
      await this.milk.approve(this.chef.address, '100', { from: bob });
      ...
      assert.equal((await this.milk.balanceOf(alice)).toString(), '700');
      assert.equal((await this.milk.balanceOf(bob)).toString(), '150');

      await time.advanceBlockTo('778');
